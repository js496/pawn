...
def update_vllm_settings(container_name, new_settings):
    """
    Update the vLLM container's settings dynamically.

    Args:
        container_name (str): Name of the vLLM container.
        new_settings (dict): Dictionary of new settings to apply.
    """
    ...
    try:
        ...
        # Update the container's environment variables and command
        container_env = container.attrs["Config"]["Env"]
        container_command = container.attrs["Config"]["Cmd"]

        # Update environment variables
        if "PYTORCH_CUDA_ALLOC_CONF" in new_settings:
            container_env.append(f"PYTORCH_CUDA_ALLOC_CONF={new_settings['PYTORCH_CUDA_ALLOC_CONF']}")

        # Update command arguments
        if "gpu_memory_utilization" in new_settings:
            container_command.extend(["--gpu-memory-utilization", str(new_settings["gpu_memory_utilization"])])
        if "max_num_seqs" in new_settings:
            container_command.extend(["--max-num-seqs", str(new_settings["max_num_seqs"])])
        if "max_num_batched_tokens" in new_settings:
            container_command.extend(["--max-num-batched-tokens", str(new_settings["max_num_batched_tokens"])])
        if "quantization" in new_settings:
            container_command.extend(["--quantization", new_settings["quantization"]])
        if "cpu_offload_gb" in new_settings:
            container_command.extend(["--cpu-offload-gb", str(new_settings["cpu_offload_gb"])])

        # Start the container with the updated settings
        container.start()
        print(f"Container '{container_name}' started with updated settings.")

    except docker.errors.NotFound as e:
        print(f"Error: Container '{container_name}' not found: {e}")
...
if __name__ == "__main__":
    ...
    update_vllm_settings("container_vllm", new_settings)
